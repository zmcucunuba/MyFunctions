...
ddd <- import("https://covid.ourworldindata.org/data/owid-covid-data.csv", format ="csv")

mis_paises <- c("Chile", "Colombia", "Peru", "Mexico", "Panama" )

ddd2 <- ddd %>% filter(location %in% mis_paises)



fun_covid_paises_1 <- function(ddd2) { 
...
fun_covid_paises_2 <- function(ddd, paises) { 
  
  ddd2 <- ddd %>% filter(location %in% paises)
...
fun_covid_paises_2(ddd, paises = c("Colombia", "Russia"))


fun_covid_paises_3 <- function(ddd) { 
  
  paises <- c("Chile", "Mexico", "Peru", "Argentina", "Panama" )
...
fun_covid_paises_3(ddd)




fun_covid_paises_4 <- function() { 
